...
//		}
		...
		if ((!empty($_POST['age']))&&(!empty($_POST['gender']))&&(!empty($_POST['weight']))&&(!empty($_POST['height'])) && (!empty($_POST['activitylevel']))){//added
			...
			if (empty($error)){
				if($row_num>0){
					$calories = getintake($gender, $weight, $height, $age, $activitylevel);
					$update = $conn->query("UPDATE user_info SET user_info.age={$age}, user_info.gender='$gender', user_info.weight={$weight}, user_info.height={$height}, user_info.activitylevel={$activitylevel} user_info.calories = {$carlories}where user_info.username = '$username'");
					
					$foodlist=food_rec($carlories,$conn);
					insert_food($foodlist,$conn,$username,TRUE);
					

				}else{
					$carlories = getintake($gender, $weight, $height, $age, $activitylevel);
					$insert = $conn->query("INSERT INTO user_info (username,age,height,weight,gender, activitylevel, calories) VALUES ('$username',{$age},{$height},{$weight},'$gender', {$activitylevel},{$carlories})");
	...
	function bmi($height, $weight){
		...
		$bmiretval = round($weight / $hsquare);
	...
	function getintake($gender, $weight, $height, $age, $activitylevel) {
		...
		if ($bmi < 18.5) {
			$intake = getintake_basic($gender, $weight, $height, $age, $activitylevel);
			$intake = $intake + 1000;
			print "You may caonsider to gain more weight to be healthy, your recommended intake calories per day is $intake: ";
		}
		else if (($bmi>18.5) && ($bmi < 24.9)) {
			$intake = getintake_basic($gender, $weight, $height, $age, $activitylevel);
			$intake = $intake + 500;
			print "Your are healthy right now, just keep up the good work. you should intake $intake kcal/day";
		}
		else {
			$intake = getintake_basic($gender, $weight, $height, $age, $activitylevel);
			print "You may consider to loes some weight. The generally accepted rate of weight loss is 1 to 1.5 pounds per week or approximately 6 pounds per month.   If you eliminate 500 kcal per day from your diet (or approximately 3500 kcal/week), you should be on track to meet this degree of weight loss. Note: there is approximately 3500 calories per one pound of fat (0.45 kg).";
		}
	}
	function getintake_basic($gender, $weight, $height, $age, $activitylevel) {
		$rmr = 0;
	...
	function food_rec($intake,$connection){
		$foodlist1=array();
		$k=0;
		$find_Carl = $connection->query("SELECT * FROM food WHERE calories < 200");
