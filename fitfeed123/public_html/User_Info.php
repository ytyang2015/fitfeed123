		...
		if ((!empty($_POST['age']))&&(!empty($_POST['gender']))&&(!empty($_POST['weight']))&&(!empty($_POST['height'])) && (!empty($_POST['activitylevel']))){//added
			...
			if (empty($error)){
				if($row_num>0){
	                ...
	                //$car_intake = 5000;
	                $car_intake=$car_intake/10;
					$update = $conn->query("UPDATE user_info SET user_info.age={$age}, user_info.gender='$gender', user_info.weight={$weight}, user_info.height={$height}, user_info.activitylevel={$activitylevel}, user_info.calories = {$car_intake} where user_info.username = '$username'");
					//$car_intake = getintake($gender, $weight, $height, $age, $activitylevel);
                    ...
                    //$car_intake = 5000;
                    $car_intake=$car_intake/10;
					$insert = $conn->query("INSERT INTO user_info (username,age,height,weight,gender, activitylevel, calories) VALUES ('$username',{$age},{$height},{$weight},'$gender', {$activitylevel},{$car_intake})");
					//$car_intake = getintake($gender, $weight, $height, $age, $activitylevel);
					$foodlist=food_rec($car_intake,$conn);
					//header('Location:recommand_result.php');
